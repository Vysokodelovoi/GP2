...
# --- ПОДСТАВЛЕНЫ ПОЛЬЗОВАТЕЛЬСКИЕ ПАРАМЕТРЫ ---
...
OWNER_ID = -132401106   # <- используй этот owner_id (отрицательный, т.к. это группа)
VERSION = "5.199"
...
def main():
    try:
        ...
        msg = str(e)
        if "15" in msg:
            print("❌ Access denied (error 15): стену можно читать только участникам сообщества или токен не имеет прав.")
        elif "5" in msg:
    ...
    for p in posts:
        ...
        rows.append({
            ...
            "date": date,
            "text_preview": text[:400],  # короткий превью
            "post_link": post_link
        })
